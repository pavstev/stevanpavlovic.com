...
# Group 3: The module path itself (e.g., "@components/ui/button.tsx").
# Group 4: The extension to remove (e.g., ".tsx" or ".ts").
# Group 5: The closing quote and optional semicolon.
REMOVE_EXT_REGEX = re.compile(r"(import(?:[\s\w,{}\*]*from\s*)?)(['\"])([^'\"]+?)(\.tsx|\.ts|\.astro)(\\2;?)")
...
def remove_explicit_extension_in_file(file_path: str):
    ...
    for line in content.splitlines():
        ...
        if match:
            ...
            module_path_raw = match.group(3) # This will be the path without extension
            extension_to_remove = match.group(4) # The .tsx or .ts
            post_import_str = match.group(5) # Closing quote and optional semicolon

            new_line = f"{pre_import_str}{quote_char}{module_path_raw}{post_import_str}"
