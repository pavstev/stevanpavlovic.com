...
class Order {
  ...
  calculateTotal(): Money {
    return this.items.reduce((total, item) => total.add(item.total), new Money(0, "USD"));
  }
}
...
class MongoOrderRepository implements OrderRepository {
  ...
  async save(order: Order): Promise<void> {
    ...
    await this.collection.replaceOne({ _id: order.id.value }, doc, { upsert: true });
  }
...
class PricingService {
  calculateOrderPrice(order: Order, customer: Customer, promotions: Promotion[]): Money {
