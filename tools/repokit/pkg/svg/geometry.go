...
func DistanceToLine(p, a, b Point) float64 {
	...
	// Perpendicular distance formula for 2D vectors
	num := math.Abs(dy*p.X - dx*p.Y + b.X*a.Y - b.Y*a.X)
	den := math.Sqrt(dx*dx + dy*dy)
	return num / den
}

// SimplifyPath implements the Ramer-Douglas-Peucker algorithm
// This recursively removes nodes that don't contribute significantly to the shape.
func SimplifyPath(points []Point, eps float64) []Point {
	if len(points) < 3 {
		return points
	}
	...
	if maxDist > eps {
		res1 := SimplifyPath(points[:index+1], eps)
		res2 := SimplifyPath(points[index:], eps)
...
// SnapPointsToAxes fixes micro-jitters in alignment by snapping to axes.
func SnapPointsToAxes(points []Point, snapAngle float64) []Point {
	if len(points) < 2 {
		return points
	}
	...
	for i := 1; i < len(result); i++ {
		...
		if math.Abs(angle) < snapAngle || math.Abs(math.Abs(angle)-math.Pi) < snapAngle {
			...
			// Snap to Vertical (90 or 270 degrees)
			result[i].X = p1.X
		}
	}
	return result
...
// FormatPathData converts points back into an optimized SVG string with Entropy Detection.
func FormatPathData(points []Point, precision int) string {
	if len(points) == 0 {
		return ""
	}
	...
	for i := 1; i < len(points); i++ {
		...
		// Optimization: Horizontal and Vertical shorthand commands
		if dy == 0 {
			sb.WriteString(fmt.Sprintf("H%g", target.X))
		} else if dx == 0 {
			sb.WriteString(fmt.Sprintf("V%g", target.Y))
		} else {
			// Entropy Calculation: Is Relative or Absolute shorter in characters?
